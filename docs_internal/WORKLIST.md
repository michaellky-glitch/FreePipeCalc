...
| TEE.1 | **Tee losses — the middle path** | **DECIDED 2026-08-26, NOT BUILT.** Michael: "take the middle path, but document it properly in the Engine page." Make only the BRANCH coefficient a function of the flow ratio Qb/Qc; leave the run flat, which is a standing CONSERVATIVE bias (it over-states, never under-states). Full write-up, tables and measurements in **`docs_internal/TEE-LOSSES.md`**; the limitation is stated in `docs/engine.html` §10 and must be updated when this lands. The hard parts are the reference-velocity conversion and the fact that a flow-dependent K makes a link's `r` a function of Q, which the Newton step does not currently assume. |
