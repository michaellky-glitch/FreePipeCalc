...
| ~~DS.1~~ | ~~**A DESIGN calculation must assume DESIGN FLOW through each piece of equipment**~~ | **DONE v0.18.15 (2026-08-25).** A CONTROLLED valve — integrated or drawn with a control link — is charged at full travel in DESIGN and at the loop's settled position in SIMULATION. A valve with no control link is a balancing valve and is read as set in both. The data hall's design index moved from AHU-4 (least remote) to AHU-13 (most remote) and no longer moves when the valves do. Michael chose the DESIGN half only; SIMULATION still ranks by worst-served. |
...
| PERF.1 | **Simulation speed on a large model — NOT TO BE WORKED ON** | Michael, 2026-08-25: "Eh not necessary. As long as it's ongoing compute and not a program error. Right now prioritize accuracy." The data hall takes 44 s and all of it is real work: 634 control solves, each repeating build + GGA up to 5 times until flow directions settle. Measured split and the three possible levers are in `HANDOVER.md` §0. Do not re-open without him. |
| DW.1 | ~~**Darcy-Weisbach in BETA**~~ | **CLOSED 2026-08-30.** Michael: "Darcy can leave beta, but change the default friction factor to colebrook-white." Done: `experimental: false`, BETA out of the name, out of the sheet note and out of the HYDRAULIC notice. **Colebrook-White is now the default correlation** for new models and for any call that does not name one — it is the reference the explicit forms are fitted to, so defaulting to a fit rather than to the thing it fits was the odd way round. Correlation names are bare (`Colebrook-White`, `Swamee-Jain`, `Haaland`, `Churchill`), no parentheticals. **Kinematic viscosity stays CONSTANT per fluid at 20 °C — his decision, "this may change in future"** (see EQ.5; it only bites under Darcy, which is now more likely to be used). An existing model keeps the correlation it was saved with. |
| TEE.1 | **Tee losses — the middle path, DARCY ONLY** | **DONE 2026-08-31. Merged to master (v0.18.32) by fast-forward; `tee-fittings` deleted.** Michael accepted the re-baseline from the data hall. Measurement in `TEE-LOSSES.md` §6C (design) and §6D (simulation). The +24.1% fitting resistance is a residue: nine riser-into-header legs up by 800 771 against 33 ordinary branch legs DOWN by 410 924, so the change came from the reference-velocity conversion and not the flow-ratio curve. Simulation is stable on all six fixtures and costs the data hall 13 s more. The data hall design index moved AHU-13 → AHU-11. **SCOPE SETTLED 2026-08-28 — option C: the flow-ratio branch coefficient goes on the DARCY path only; Hazen-Williams is left as-is and warns instead (shipped v0.18.23). The solver already sums `r·Q^n + rK·Q²` and Darcy already carries `rK`, so the architecture exists.** Still to build: the Rennels Ch 16 branch curve. **DECIDED 2026-08-26, NOT BUILT.** Michael: "take the middle path, but document it properly in the Engine page." Make only the BRANCH coefficient a function of the flow ratio Qb/Qc; leave the run flat, which is a standing CONSERVATIVE bias (it over-states, never under-states). Full write-up, tables and measurements in **`docs_internal/TEE-LOSSES.md`**; the limitation is stated in `docs/engine.html` §10 and must be updated when this lands. The hard parts are the reference-velocity conversion and the fact that a flow-dependent K makes a link's `r` a function of Q, which the Newton step does not currently assume. |
| TEE.2 | **The Darcy K table holds the 1993 values, not ASHRAE's newer measurements** | Found 2026-08-26 from Ch 22 Table 7. `data/ktable.js` THREADED carries TBRANCH 1.4 at 2 in and TRUN 0.90 flat — exactly Table 7's "Past" column (Crane 1988 / Freeman 1941 / Hydraulic Institute 1990, published 1993). The same table's ASHRAE Research columns measure **0.93 branch and 0.19 line** for a 50 mm thread tee. The run is over-charged by about **5x** at the flow-through end, which is the commonest case on a header. NOT a drop-in swap: the research columns are velocity-dependent and sparse, cover thread at 50 mm and weld above it, and the engine has no welded set. Changing them re-baselines every Darcy answer. **Michael's decision, independent of TEE.1.** Detail in `TEE-LOSSES.md` §6B. |
| DP.1 | **A DP-controlled pump cannot hold its stated setpoint — possible bug** | Michael, 2026-08-28, from building Tutorial 02: with DPS-01 between the AHU-5 supply and return set to **200 kPa** and PMP-1 linked to it, the simulation FAILS to hold it. Raising the setpoint by **typically 50 kPa** (to 250 kPa, which is what the shipped `examples/Tutorial 02 - Hydronic System.json` carries) settles it. **His hypothesis: the heat-exchanger control valves add pressure drop the sensor does not see, so the pump is asked for a differential that does not exist at the sensor's tapping points.** MEASURED 2026-08-28 on the shipped file: 150 kPa -> pump 75%, 200 -> 86%, 250 -> 95% settles, 300 -> 100% at-max. **The tell: at 150 and 200 kPa the pump is NOT at its ceiling (75%, 86%) and the device reports state `on`, yet `SETPOINT_LOST` fires anyway** — a state/error contradiction that is itself the thread to pull. A pump with headroom that reports the setpoint lost is either mis-judging `lost`, or the sensor is reading a differential the pump genuinely cannot create at that tapping regardless of speed. May not be a bug — it may be correct behaviour that the tutorial should simply explain. **To be traced the week of 2026-08-31.** Reproduce by loading the shipped file, setting `DPS-01` to 200 kPa and re-simulating. |
| TH.1 | **Chillers hit a brick wall instead of overloading gracefully** | Michael, 2026-08-29, from `debug/network.pnet(7).json`. A capacity-limited source/sink stops dead at its nameplate, so the loop temperature runs away until pipe heat exchange with ambient balances the deficit. **Real chillers do not behave like this — they can run to roughly 110% in an overloaded condition, drawing more power.** The REQUIRED-CAPACITY half of this is FIXED in v0.18.29 (it is now asked of a plant with the ceilings lifted, so the reported figure is the duty the building presents, not the runaway it causes). **MEASURED 2026-08-29, and Michael's physics is EXACTLY what the program does.** Ambient is 20 °C. LIMITED: the loop settles at 16.76–22.77 °C, straddling ambient, and net pipe exchange is **0.00008 kW** — the loop rose until gain and loss cancelled, his second regime. UNLIMITED: the loop sits at 7.50–13.43 °C, entirely below ambient, and picks up **+0.22 kW**, his first regime. **The two chillers are NOT symmetric**: flows 16.132 vs 16.181 L/s (0.3% apart), so at the same 5.93 K they need **399.50 and 400.72 kW**. Total required 800.22 kW against 800.00 installed — the plant is **0.72 kW short on WCH02 alone**, and because they share one loop WCH02's shortfall drags the loop up until WCH01 caps out too. So each figure is individually right, and the SPLIT is what makes one read below 400. **THE REPORTING DEFECT THAT REMAINS:** the panel shows `Limited by Capacity`, `Setpoint deficit +8.48 K` and `Margin on selection +0.1%` together. A machine that is capacity-limited and 8.5 K off setpoint cannot sensibly show a POSITIVE margin — it is failing because its neighbour is short, not because it is. Requirement here is a SYSTEM property and the per-machine margin misleads. **DECIDED 2026-08-30 — OPTION 1: report a PLANT-LEVEL shortfall.** When the aggregate cannot reach setpoint, say so at the loop level and mark every machine in it as bound by that, instead of showing each a private margin. **The message already exists**: `EQUIP_LIMITED` fires on both chillers ("... is limited by Capacity and is not reaching its setpoint"), so this is not about detecting the condition — it is about the panel not contradicting it. The per-machine `Margin on selection` is the thing to change; splitting the requirement by flow (option 2) was rejected because it hides which machine is actually short. **Still to do: a graceful overload — let the machine exceed its nameplate by a stated margin, report the loss of setpoint plainly, and stop the runaway being the only outcome.** Needs Michael's steer on the overload margin and on whether power draw should be modelled at all. |
