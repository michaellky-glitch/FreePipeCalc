...
| `LAMINAR` | \[Pipe Tag\] is in laminar flow (Re = \[Re\]). Hazen-Williams calculation method is not reliable in this region. Consider using Darcy-Weisbach calculation method. | Re below 2000: the wrong equation, not just imprecise — a warning about the METHOD. |
| `TRANSITIONAL` | \[Pipe Tag\] is in transitional range (Re = \[Re\]). Both friction calculations are unreliable in this region. Consider changing pipe size. | Re 2000–4000; uncertain whatever the method. |
