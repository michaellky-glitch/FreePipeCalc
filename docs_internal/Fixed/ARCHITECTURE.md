...
| `data/fittings.js` | Fitting equivalent lengths from published tables (Carrier, NFPA 13, custom) keyed on nominal size; K-based losses. |
...
| `data/pumps.js` | Pump curves: three-point quadratic, least-squares fit, affinity scaling, parsing. |
...
On the HYDRAULIC tab, all K values and the Custom equivalent-length table are
editable. The published EL tables (Carrier, NFPA 13) are read-only, because
jurisdictions differ and the built-ins are a starting point, not an authority —
if you need different equivalent lengths, switch the set to Custom and type them.
...
Seven suites, 1681 assertions, no dependencies:
