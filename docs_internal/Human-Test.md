...
| EQ.3 | **Tee losses where flow divides or combines.** | **ANSWERED 2026-08-26 — the MIDDLE PATH: make only the BRANCH coefficient a function of Qb/Qc, leave the run flat.** Idelchik rejected as a third provenance on top of ASHRAE and Carrier. Still the largest known approximation in the engine, and **DECIDED BUT NOT BUILT**. Everything is in **`docs_internal/TEE-LOSSES.md`** — what the engine charges today (both tables), the two separate approximations, the measured exposure (tees are **39 % of effective length on the Data Hall**), why leaving the run flat is conservative, and the two hard parts of building it. |
