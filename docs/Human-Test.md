...
| DW.O | **Default flow/pressure remapped to your 604.3 sheet** (v0.17.9) | ⬜ | Each fixture/variation's default design flow & pressure now follows your spreadsheet exactly (WC priv-tank→flushometer tank 1.6 gpm, priv/pub-valve→blow-out 25 gpm/45 psi, pub-tank→close-coupled 3 gpm; urinals→urinal valve 12 gpm; dishwashing machine added). Estimates (bathroom group, kitchen sink public, shower public, washing machines) are computed from 604.3 and shown in **red** with your footnote on the HYDRAULIC tab's new "Fixture design flow & pressure" table. **Confirm the estimates and the WC/urinal outlet choices — these are the numbers that drive sizing & simulation.** The IPC data stays verified:false. |
