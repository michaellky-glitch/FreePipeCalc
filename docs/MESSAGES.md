...
| `DW_GENERIC_DEMAND` | … generic outflows add … L/s to the design flow UNDIVERSIFIED (…). A generic outflow is a continuous draw, not a fixture unit, so it is added on top of the fixture-unit flow rather than through the demand curve. Check the figure is one you chose — 1.00 L/s is the default a new outflow is created with. | A NOTICE, not a warning: the rule is correct and deliberate. It exists because the number is easy to leave at its default and is added linearly to every pipe upstream — on 20260818-lowrise one such node was a quarter of the building's design flow. |
| `DW_REMOTE1` | REMOTE1: … of … fixtures are served at once, opened most-remote first — opening … is what the system cannot deliver. | A NOTICE carrying the result of the REMOTE1 load case (a WARNING when not even the most remote fixture can be served on its own). Raised by `network.plumbingRemote1`, which is the plumbing SIMULATE method — see docs/DW-MODULE.md → "REMOTE1". |
