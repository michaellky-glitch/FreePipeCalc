...
| `THERMAL_LIMIT` | Temperature at \[Node\] solves to \[Temperature\] \[Unit\], outside the limits set in THERMAL. Check the heat balance of the system, or widen the temperature range in the THERMAL tab if correct. | A solved temperature is outside `thermal.tempMin … tempMax` — the runaway guard. The solve is exact; a correct answer can still be absurd. |
...
| `CV_FLOW_LIMIT` | \[Tag\] is operating beyond its Flow Limit (\[Limit\]). Change the operating conditions around it, or warning threshold in Hydraulics. | A control valve passing more than the nominal flow its selected size is rated for. The threshold is `warn.cvFlowLimit`, a multiple of rated flow, editable on HYDRAULIC. Only a valve with a SIZE selected has a rating to breach. What happens past the limit is deliberately not modelled. |
| `CV_MIN_DP` | \[Tag\] is operating below minimum dP (\[Limit\]). Change the operating conditions around it, or warning threshold in Hydraulics. | A control valve with less differential across it than it needs to regulate with: wide open, and the branch taking whatever the system gives it. The threshold is `warn.cvMinDp` on HYDRAULIC and ships at **0, meaning off** — the data sheets state a maximum differential and no minimum, so there is no published figure to default it to. A valve passing no flow is not reported. |
...
| `ICV_EXCEEDS_PD` | \[Tag\] has a control valve that drops more than the whole rated pressure. Raise the valve Kv or the rated pressure drop. | The rated dP of a machine is the TOTAL for the branch, coil and integrated control valve together, so the coil's own resistance is the rating LESS the valve's full-open drop at rated flow. A Kv small enough to spend the whole rating leaves no coil. The coil is floored at zero and the machine becomes its own valve. |
...
| `PDM` | \[Pipe Tag\] friction rate \[PDM\] \[Units\] exceeds \[PDM\] \[Units\] set in HYDRAULIC. | Over `warn.pdm` (400 Pa/m), the usual sizing criterion. |
| `EQUIP_AUTO_SIM` | \[Equipment Tag\] is in Auto Mode. Simulating with Equipment in Auto Mode assumes that the pressure drop is sized exactly to the current conditions. | Michael, 2026-08-29. A heat source/sink on AUTO sizing is unlimited and its rated point is whatever the solve landed on, so its resistance is derived from the very conditions being simulated — in effect a machine selected exactly for this moment. Correct for DESIGN, where finding the selection IS the job; worth saying in SIMULATION, where the reader is asking what a real plant does at part load. Raised per machine, SIMULATION only. |
...
| `LAMINAR` | \[Pipe Tag\] is in laminar flow (Re = \[Re\]). Hazen-Williams calculation method is not reliable in this region. Consider using Darcy-Weisbach calculation method. | Re below 2300: the wrong equation, not just imprecise — a warning about the METHOD. Not raised for a pipe whose flow prints as 0.00 L/s: the warning is about an unreliable friction loss, and the loss through a pipe carrying nothing is nothing. |
| `TRANSITIONAL` | \[Pipe Tag\] is in transitional range (Re = \[Re\]). Both friction calculations are unreliable in this region. Consider changing pipe size. | Re 2300–4000; uncertain whatever the method. Not raised for a pipe whose flow prints as 0.00 L/s: the warning is about an unreliable friction loss, and the loss through a pipe carrying nothing is nothing. |
...
| `EQUIP_LIMITED` | \[Tag\] is limited by \[Parameter\] and is not reaching its setpoint. | A source/sink pinned at its capacity ceiling and STILL short of setpoint — a failure, not a margin. Raised for `Capacity` and `Capacity (wrong direction)` only: a machine held by its Design ΔT or by a temperature limit is constrained but is not out of capacity, and is not reported. Named per machine, so on a plant of several the one that is short is the one you see. |
| `FLUID_TEMP_RANGE` | Temperature is out range (3–79 °C). We may revisit if heating becomes a use case. | The model's water temperature is outside the band this program is built for. It comes from the SOURCE (see the Calculation Method page, section 5.1). Density is checked against its source data to 100 °C, but VISCOSITY only to 80 °C, so outside the band the viscosity is an extrapolation nobody has verified and the friction is not trustworthy. Chilled water and low-temperature hot water both sit inside it. Not raised for a glycol or custom fluid, which carries its own stated properties and extrapolates nothing. |
| `EQUIP_OVERLOAD` | \[Tag\] is overloaded, running at \[%\] of its capacity. | A source/sink doing MORE than its nameplate and still holding its setpoint. This is the overload allowance being used (THERMAL ▸ Overload capacity, 10% by default), not a failure. Raised against the NAMEPLATE, per machine — on a plant at full load a fraction of a per cent of flow imbalance puts one unit over, and that unit is the one named. |
